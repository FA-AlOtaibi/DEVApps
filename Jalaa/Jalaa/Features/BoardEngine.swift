...
enum BoardEngine {
    static func nodes(from source: String) -> [BoardNodeData] {
        ...
        return Array(sentences.prefix(8)).enumerated().map { index, sentence in
    ...
    static func explain(_ node: BoardNodeData) -> String {
        "ببساطة: \(node.detail)"
    }

    static func deepen(_ node: BoardNodeData, source: String) -> String {
        let related = nodes(from: source).filter { $0.id != node.id }.prefix(2).map(\.detail)
    ...
    static func example(_ node: BoardNodeData) -> String {
        "مثال تطبيقي: تخيّل موقفًا واقعيًا تنطبق فيه الفكرة التالية: «\(node.detail)». حاول تحديد السبب والنتيجة بنفسك ثم ارجع للنص للتحقق."
    }
